...
5. **Seam reconstruction — inline reveal + fixed inspector (keyboard-driven, no hover).** The **active seam** — set by the arrow-key walk (Step 6) — expands its predicted text inline (dimmed, in the reading type) and fills the fixed inspector at the bottom of the middle column with the actual text + fidelity (**State 6**). **No hover/rollover trigger** (it misfires when the cursor merely rests over the text during keyboard use, reading as a bug), no floating card, no click-to-pin. Steps 5 and 6 are tightly coupled: Step 5 builds the reveal presentation given an active seam; Step 6 wires the keys that set it.
