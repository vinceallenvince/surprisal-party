...
**Why this is no longer an "ugly but functional" interim.** The original plan called for a deliberately unstyled build with all styling deferred to Phase 3. That made sense when the visual design was still unknown and would be *discovered* by building. It no longer holds: the design is fixed — a high-fidelity Figma Make prototype and a set of static Figma Design frames, one per scenario/state (see `process.md`, Step 6). Building unstyled then restyling would be wasted motion and would invite drift from the established design. So Phase 2 builds **styled from the first commit, directly against the frames**, sequenced by **dependency and risk** rather than by fidelity. (An earlier draft treated the gap-closing reflow as a hard FLIP "spike" — the project's biggest animation risk. Inspecting the Figma Make prototype retired that: it achieves the felt transition with plain CSS transitions, not FLIP. See Step 4.)
...
4. **Gentle state transitions (CSS only — no FLIP, no word flight).** Soften the position-to-position change so it reads as a transition rather than a hard snap. Words do **not** literally travel between the prose and the strip — that expectation is explicitly dropped. The prototype confirmed a light CSS treatment is enough: the reflow happens instantly under a brief cross-dissolve / colour ease. Two viable approaches (the prototype uses a per-word colour ease that is partly an artifact of index-keying; our runtime keys words by stable identity, so we add the transition intentionally):
   - **(2) State cross-dissolve — current choice.** Keep kernel colouring constant (highlighted from position 0). On each slider step, apply a short CSS opacity/colour ease to the prose column, right strip, and header readout so one state softly dissolves into the next. Least logic, no data change. Honour `prefers-reduced-motion` (no transition when set).
   - **(1) Per-word colour warming — alternative.** Make the kernel highlight threshold-progressive and `transition-colors` each word, so surviving words warm into coral as they become load-bearing. Closer to the prototype's look; a small departure from the "kernel highlighted from position 0" rule. Deferred unless (2) feels too uniform.
   No `getBoundingClientRect`-based layout animation, no transforms, no FLIP.
