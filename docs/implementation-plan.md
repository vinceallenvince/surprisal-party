...
**Why this is no longer an "ugly but functional" interim.** The original plan called for a deliberately unstyled build with all styling deferred to Phase 3. That made sense when the visual design was still unknown and would be *discovered* by building. It no longer holds: the design is fixed — a high-fidelity Figma Make prototype and a set of static Figma Design frames, one per scenario/state (see `process.md`, Step 6). Building unstyled then restyling would be wasted motion and would invite drift from the established design. So Phase 2 builds **styled from the first commit, directly against the frames**, sequenced by **dependency and risk** rather than by fidelity. (The gap-closing reflow is the project's main animation risk and is isolated in Step 4. We explored CSS-only alternatives — a state cross-dissolve and per-word colour warming — but neither delivered the "surviving words collapse into the gaps" feel, so Step 4 commits to a real FLIP reflow via `framer-motion`. See Step 4.)
...
4. **Gap-closing reflow (FLIP via `framer-motion`).** When the slider moves, the surviving words **physically slide to close the gaps** left by removed spans, rather than snapping into a new layout — this *is* the felt sense of the prose compressing. Implemented with **framer-motion layout animations**: each prose word (and seam) is a `motion.span` with the `layout` prop, so the library performs the FLIP (measure first/last, invert, play), including multi-line reflow and interruption handling on fast slider drags.
   - **Prose-only.** Words do **not** fly between the prose and the right strip — that expectation was dropped. Removed words fade/shrink out as the survivors close in; tiles simply appear in the strip.
   - **Both directions.** Compression closes gaps (survivors slide together); decompression opens them (survivors slide apart as returning words fade in). Stable word keys (`w<index>`) give framer-motion the identity it needs to track each survivor across positions.
   - **Reduced motion.** Honour `prefers-reduced-motion` — no layout animation, instant swap.
   - **Performance.** ~1,300 words, and a large slider jump moves hundreds at once; keep the motion calm (no bouncy easing) and cap duration / consider a shorter animation on big jumps if it janks. Adds the `framer-motion` client dependency.
