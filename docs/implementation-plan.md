...
## Future exploration — semantic fidelity (not scheduled)

A discussion-stage idea, captured for later; nothing here is committed to build.

**The observation.** The compression↔prediction equivalence is *exact* at the symbol level (Shannon source coding / arithmetic coding: optimal code length is `−log p(exact token)`, and surprisal — which drives the fade — is exactly that). But the *reconstruction* the app performs is **lossy** compression, and lossy compression is only defined relative to a **distortion measure** (rate–distortion theory). Our current fidelity metric is **token-set F1** ([`pipeline/cprediction/fidelity.py`](../pipeline/cprediction/fidelity.py)) — purely *lexical* surface overlap. It credits exact word matches and (being a bag of words) is order-insensitive, but synonyms, paraphrase, and inflection all score ~0. So we are measuring a lossy phenomenon with a near-lossless (exact-token) ruler, which makes the mechanic read as "*very* lossy" when much of the *meaning* may be conserved.

**Why semantic similarity is the more aligned lens.** Humans run compression↔prediction at the level of meaning, not surface form: a good book title compresses a book precisely because it lets you *predict* its content; memory is reconstructive and gist-based, not verbatim. Under a **semantic** distortion measure, the app's actual thesis lands — *the words are discarded, the meaning is conserved* — which is a truer and more compelling rendering than "everything is lossy." It would likely also **vindicate the abstract's original "lighter left, fidelity ≈ high" intuition**: early reconstructions do preserve meaning; it was only lexical F1 that scored them ~0.2. "Our expectations were off" may really have been "we measured the wrong distortion."

**Sketch, if pursued.** Semantic similarity is deterministic and **precomputable at build time** (embed `predicted` vs `actual`, score, store per gap) — so the runtime stays static, the hard architectural commitment. Concretely: a semantic scorer in the pipeline (sentence-embedding cosine, or **BERTScore** — the embedding-matched analogue of the token-F1 we already have), a new `GapEntry` field (e.g. `semantic_fidelity`), a schema minor-version bump + cache regeneration, and a runtime readout.

**The real design question is *surfacing*, not computing.** The payoff is the **contrast** between lexical and semantic fidelity (word-fidelity ~0.2, meaning-fidelity ~0.8 → "form discarded, meaning kept"), not a lone extra number. Options: (a) two fidelity numbers in the header (`word-fidelity · meaning-fidelity`, a nice pairing alongside `stored · removed`, but busier and needs relabeling "avg fidelity" → "word fidelity"); (b) replace lexical with semantic (calmer, but loses the contrast and the "1.00 = exact" anchor); (c) keep the header lexical and show semantic in the reconstruction inspector, where the contrast appears as you inspect a gap.

**Caveats.** (1) Short gaps stay noisy — embedding similarity of a 1–2 word span is unreliable, so semantic fidelity is most meaningful on larger spans, making this **synergistic with the parked gap-granularity work** (do them together). (2) A semantic score is model-judged — one model's verdict on another's output — in mild tension with the project's *verify-by-recognition* ethos; the actual reconstruction stays visible to check, but the metrics modal should say the number is model-based. (3) `1.00 = exact` stops being universal (semantic 1.0 ≠ identical text), so the two metrics need distinct definitions ("same words" vs "same meaning").
