...
- **No focus contention:** the arrow keys *always* drive the seams — there is no need to scope them to a focused column. The **slider is mouse-only** (drag the thumb; no arrow-key nudging), so the two controls never fight over the same keys.
- The active seam carries a quiet but unambiguous **indicator** (the expanded predicted text plus a subtle ring/underline) so the user always knows where they are in the walk.
- **Discoverability hint:** the arrow-key affordance isn't obvious, so the first time the slider moves to any position past UNCOMPRESSED, a small hint bubble appears hovering just **above the first seam** with concise copy like **"Use arrow keys"** (plus a tiny ← → glyph). It's a quiet tooltip-style chip, not a modal. It dismisses on first arrow-key press (or after a few seconds / on Esc), and doesn't reappear once the user has stepped through a seam in this session. Keep it unobtrusive — it's a nudge, not a tutorial.
